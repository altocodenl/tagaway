...
var svg = {
   ...
   play: '<svg class="play" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 5.14v13.72L19 12L8 5.14z" fill="currentColor"/></svg>',
   pause: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" fill="currentColor"/></svg>'
}
...
B.mrespond ([
   ...
   ['query', 'pivs', function (x, options) {
      ...
      B.call (x, 'post', 'query', {}, {
         ...
         if (rs) clog ('DEBUG PERF', rs.body.perf);
   ...
   ['slideshow', 'start', function (x) {
      if (B.get ('State', 'slideshowInterval')) return;
      B.call (x, 'set', ['State', 'slideshowInterval'], setInterval (function () {
         B.call (x, 'open', 'next');
      }, 3000));
   }],
   ['slideshow', 'stop', function (x) {
      var interval = B.get ('State', 'slideshowInterval');
      if (interval) clearInterval (interval);
      B.call (x, 'rem', 'State', 'slideshowInterval');
   }],
   ['change', ['State', 'slideshow'], {match: B.changeResponder}, function (x) {
      B.call (x, 'slideshow', B.get ('State', 'slideshow') ? 'start' : 'stop');
   }],
   ['enter', 'fullscreen', function (x) {
      if (B.get ('State', 'slideshow')) B.call (x, 'slideshow', 'start');
   ...
   ['open', /prev|next/, function (x) {
      ...
      // Clear timers
      if (B.get ('State', 'slideshow')) {
         B.call (x, 'slideshow', 'stop');
         B.call (x, 'slideshow', 'start');
      }
   }],
...
views.logo = function (size) {
   return [
      ['span', {style: style ({'font-weight': 'bold', color: '#5b6eff', 'font-size': size})}, 'tagaway'],
   ];
}
   ...
   return B.view ([['State', 'date'], ['State', 'selected']], function (date, selected) {
      ...
      return ['div', {class: 'feedback-box-mask'}, [
         ['div', {class: 'change-date'}, [
            ...
            ['div', {style: style ({float: 'right', 'margin-right': '10px'})}, [
         ...
         ['div', {class: 'auth-card'}, [
            ['div', {class: 'auth-card__inner'}, [
               ['div', {class: 'auth-card__header'}, [
                  ...
                  ['a', {class: 'sign-up-in-with-google-a', href: googleURI}, [
                     ['span',{class: 'sign-up-in-with-google-span'}, [
                        [H.putSvg('googleGIcon')]
                     ]],
                  ...
                  ['a', {class: 'sign-up-in-with-google-a', href: appleURI}, [
                     ...
                     ['p', {class:'sign-up-in-with-google-p'}, 'Sign in with Apple'],
                  ]],
               ]],
            ...
            ['div', {class: 'auth-card__inner'}, [
               ['div', {class: 'auth-card__header'}, [
                  ...
                  ['a', {class: 'sign-up-in-with-google-a', href: appleURI}, [
                     ...
                     ['p', {class:'sign-up-in-with-google-p'}, 'Sign up with Apple'],
                  ]],
               ]],
               ['form', {onsubmit: 'event.preventDefault ()', class: 'enter-form auth-card__form'}, [
                  ...
                  ['div', {class: 'sidebar__inner-section'}, [
                     ...
                     B.view ([['State', 'filter'], ['State', 'query', 'tags'], ['Data', 'queryTags'], ['Data', 'monthTags'], ['Data', 'account'], ['State', 'showNTags'], ['State', 'tagOrder'], ['State', 'expandCountries'], ['State', 'expandYears']], function (filter, selected, queryTags, monthTags, account, showNTags, tagOrder, expandCountries, expandYears) {
                        ...
                        var makeTag  = function (which) {
                           ...
                           if (H.isUserTag (which)) Class += ' tag-actions-rename-tag';
                           var numberOfPivs;
                           ...
                           return ['li', {class: Class, style: disabledTag ? 'cursor: default' : undefined, onclick: disabledTag ? B.ev (H.stopPropagation) : B.ev (H.stopPropagation, action, action2 || ['foo', 'bar'])}, [
                              ...
                              ['div', {class: 'tag__actions', style: style ({height: 24})}, [
                                 inc (['f::', 'e::'], which) ? [] : ['div', {class: 'tag-actions'}, [
                                    ...
                                    ['div', {onclick: B.ev (H.stopPropagation, ['rename', 'tag', which])}, H.putSvg ('renameTag', 24)],
