...
if (cicek.isMaster) a.stop ([
   ...
   // Compute hashes without metadata & get metadata using exiftool
   function (s) {
      ...
      a.seq (a.creat (), [
         [a.fork, dale.go (s.pics, function (pic) {return {id: pic.id, owner: pic.owner, vid: pic.vid, ext: Path.extname (pic.name)}}), function (pic, index) {
            var path     = Path.join (CONFIG.basepath, H.hash (pic.owner), pic.id);
            var hashpath = Path.join ('/tmp', pic.id + pic.ext);
            return [
               [a.log, 'start #' + (index + 1) + '/' + s.pics.length],
               pic.vid ? [a.stop, [k, 'ffmpeg', '-i', path, '-map_metadata', '-1', '-c:v', 'copy', '-c:a', 'copy', hashpath], function (s, error) {
                  if (error.code !== 0) return s.next (null, error);
                  s.next ();
               }] : [
                  [a.make (fs.copyFile), path, hashpath],
                  [k, 'exiftool', '-all=', '-overwrite_original', hashpath],
               ...
               function (s) {
                  fs.readFile (hashpath, function (error, file) {
                     ...
                     hashes [hash].push (pic.id);
                     s.next ();
                  });
               },
               [a.make (fs.unlink), hashpath],
               [k, 'exiftool', path],
               [a.log, 'end.. #' + (index + 1) + '/' + s.pics.length],
            ];
         }, {max: 8}],
         function (S) {s.hashes = hashes; s.next (S.last)}
      ]);
   },
   ...
   function (s) {
      ...
      dale.go (s.pics, function (pic, k) {
         ...
         if (error) errors.push ({pic: pic.id, error: error});

         var rotation = dale.stopNot (metadata, undefined, function (line) {
            if (line.match (/^Orientation/)) return line;
         }) || '', deg;

         if      (rotation.match ('270')) deg = -90;
         else if (rotation.match ('90'))  deg = 90;
         else if (rotation.match ('180')) deg = 180;
         if (deg && ! pic.deg && deg !== parseInt (pic.deg)) {
            ops.push (['fix rotation', pic.id, deg]);
            multi.hset ('pic:' + pic.id, 'deg', deg)
         }
         ...
         pic.dates = JSON.parse (pic.dates);

         if (date !== pic.date) {
            if (date > pic.date) clog ('WARNING, OLDER DATE IGNORED', pic.id, date - pic.date, new Date (date), new Date (pic.date));
            ops.push (['date', pic.id, date, pic.date]);
            multi.hset ('pic:' + pic.id, 'date', date);
         }
         if (! teishi.eq (pic.dates, dates)) {
            ops.push (['dates', pic.id, dates, pic.dates]);
            multi.hset ('pic:' + pic.id, 'dates', JSON.stringify (dates));
         }
      });
      if (errors.length) clog ('ERRORS', errors);
      dale.go (s.hashes, function (ids, hash) {
         if (ids.length > 1) {
            // Assumes repeated files come in pairs.
            if (pics [ids [0]].owner === pics [ids [1]].owner) ops.push (['delete repeated', pics [ids [1]].owner, ids [1]]);
         }
         if (pics [ids [0]].hash !== hash) {
            // Ignores upicd
            ops.push (['update hash', pics [ids [0]].owner, ids [0], hash]);
            multi.hset ('pic:' + ids [0], 'hash', hash);
            multi.sadd ('upic:' + pics [ids [0]].owner, hash);
            multi.srem ('upic:' + pics [ids [0]].owner, pics [ids [0]].hash);
         }
      });
      dale.go (ops, function (op) {
         clog (op);
   ...
   function (s) {
      a.fork (s, dale.fil (s.ops, undefined, function (op) {
         if (op [0] === 'delete repeated') return op;
      }), function (op) {
         return [H.deletePic, op [2], op [1]];
      });
   },
   function (s) {
      notify (s, {priority: 'critical', type: 'Script to fix orientation, dates & repeated pics/vids success.', ops: s.ops});
   }
], function (s, error) {
   notify (s, {priority: 'critical', type: 'Script to fix orientation, dates & repeated pics/vids error.', error: error});
});
